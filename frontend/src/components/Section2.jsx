...
const FeatureCard = ({ icon, title, description, delay }) => {
    return (
        ...
        >
            ...
            <h3 className = {`${playfair.className} text-3xl font-thin text-white text-left mb-4`}>
                {title}
            </h3>
            <p className = {`${openSans.className} text-[#FCF9E2] text-left font-medium leading-[1.7rem] opacity-80`}>
                {description}
            </p>
        </motion.div>
    );
    ...
    return (
        <section id = "section-2" className = "py-16 min-h-screen bg-[#101921]">
            <div className = "container mx-auto px-4 flex flex-col gap-2">
                ...
                >
                    <motion.h2
                        className = {`${playfair.className} text-4xl md:text-6xl text-white mb-4 text-left font-thin`}
                        initial = {{ y: -20 }}
                        animate = {{ y: 0 }}
                    ...
                    <motion.p
                        className = {`${openSans.className} text-[#FCF9E2] max-w-3xl mx-auto text-right font-medium leading-[2.2rem] mb-12`}
                        initial = {{ opacity: 0 }}
                        animate = {{ opacity: 1 }}
                    ...
                    {features.map((feature, index) => (
                        <React.Fragment key = {index}>
                            ...
                            {index < features.length - 1 && (
                                <div className = "hidden md:block h-auto self-stretch w-px bg-gradient-to-b from-transparent via-[#A8DADC] to-transparent opacity-40 mx-4" />
                            )}
                        </React.Fragment>
                    ))}
                </div>
