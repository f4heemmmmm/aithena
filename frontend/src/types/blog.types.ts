// Fixed blogService.ts with TypeScript compliance
...
class ApiServiceError extends Error {
    ...
    constructor(message: string, status_code: number, originalError?: unknown) {
        super(message);
...
class BlogService {
    ...
    // Admin methods with proper error handling
        ...
        try {
            ...
            if (axiosError.response?.status === 404) {
                try {
                    ...
                    if (query?.categories && query.categories.length > 0) {
                        query.categories.forEach(category => queryString.append("categories", category));
                    }
