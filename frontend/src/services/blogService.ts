...
class BlogService {
  ...
  private setupInterceptors(): void {
    ...
    // Response interceptor for authenticated requests only
    ...
    // Only add interceptor to authenticated API
  ...
  /**
   * Try multiple possible API endpoints without logging expected failures
   */
  private async tryApiEndpoints<T>(endpoints: string[]): Promise<T> {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        // Only log if it's not a 404 (expected when trying multiple endpoints)
  ...
  getExcerpt(post: BlogPost, maxLength: number = 150): string {
    ...
    // Strip HTML tags and get first part of content
