...
const TeamSection: React.FC<{ section: TeamSection }> = ({ section }) => {
  ...
  const renderCategories = (categories: { [key: string]: TeamMember[] }) => {
    ...
    // Dynamic grid classes based on number of categories
    const getGridClasses = () => {
      if (categoryCount === 1) return "grid grid-cols-1 max-w-md mx-auto";
      if (categoryCount === 2) return "grid grid-cols-1 md:grid-cols-2 max-w-2xl mx-auto divide-y md:divide-y-0 md:divide-x divide-gray-100";
        ...
        <div className={getGridClasses()}>
          {categoryEntries.map(([category, members]) => (
            <div key={category} className="p-8 text-center">
              <div className="mb-6">
                <h4 className={`${dmSans.className} font-semibold text-gray-900 text-lg tracking-wide uppercase text-sm`}>
                  {category}
                </h4>
                <div className="mt-2 w-12 h-0.5 bg-blue-600 mx-auto"></div>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className="p-8 lg:p-12">
            <div className="text-center mb-10">
              <h2 className={`${dmSans.className} text-4xl font-bold text-gray-900 mb-4`}>
                {section.title}
              </h2>
              <div className="w-20 h-1 bg-blue-600 mx-auto"></div>
            </div>
            
            <div className="prose prose-lg max-w-none text-gray-600 mb-12 leading-relaxed text-left">
              {section.description.split('\n\n').map((paragraph, index) => (
                <p key={index} className={`${inter.className} mb-6 text-lg leading-8`}>
                  {paragraph}
                </p>
              ))}
            </div>
      ...
      <div className="bg-gradient-to-br from-blue-50 via-white to-indigo-50 py-24">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8 text-center">
            <h1 className={`${dmSans.className} text-5xl font-bold tracking-tight text-gray-900 sm:text-6xl mb-6`}>
              Meet the Team
            </h1>
            <div className="w-24 h-1.5 bg-blue-600 mx-auto"></div>
          </div>
          
          <div className="prose prose-xl max-w-none text-gray-600 leading-relaxed text-left">
            <p className={`${inter.className} mb-6 text-xl`}>
